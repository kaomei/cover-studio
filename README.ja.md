...
クリエイター、個人開発者、テックブロガーのためのオープンソース AI Skill。初期設定で出力モードを確認し、記事入力時に**3種類のデザイン案を同時提案**。生成前に**大見出し・副見出し・タグ要素の文字階層を必ず確認**します。ブランドシステム（Brand System）固定化機能に加え、**Codex 環境直接生成**と **Nano Banana (Google Flow 無料) プロンプト出力**に対応。
...
- ⚙️ **初回確認フロー**：複数サイズ一括生成（3:4, 2.35:1, 16:9等）か単一サイズ個別指定かを事前に確認。
- 🧠 **3パターン同時提案**：記事内容を自動分析し、異なる3つの高品質デザイン構図を同時に提案。
- ✍️ **文字階層の必須確認ステップ**：大見出し、副見出し、タグ要素の文言を生成前に必ずユーザーと擦り合わせ。
...
```mermaid
flowchart LR
    A[フェーズ 0: 出力モード初期設定] --> B[フェーズ 1: 記事入力]
    B --> C[フェーズ 2: 3案同時提案]
    C --> D[フェーズ 3: 案選定＆文字階層の確認]
    D --> E[フェーズ 4: Codex直出 / Flow無料生成]
```
