...
クリエイター、個人開発者、テックブロガーのためのオープンソース AI Skill。初期設定で出力モードを確認し、記事入力時に**大見出し・副見出し・タグ要素の文字階層をまず確認**。その文字を基に**3種類の異なるデザイン画像を直接生成・提示**します。ブランドシステム固定化機能に加え、**Codex 環境直接生成**と **Nano Banana (Google Flow 無料) プロンプト出力**に対応。
...
flowchart TD
    subgraph S0["⚙️ 初期設定 (初回のみ)"]
        A0[Skill起動] --> A1[出力モード確認:<br/>1. 🌟 ブランド統一マルチサイズ一括出力<br/>2. 🎯 単一サイズ個別カスタム]
    end
    ...
    subgraph S3["🚀 フェーズ3: 選択＆最終納品"]
        ...
        D1 -->|マルチサイズモード| D2[全プラットフォームサイズを一括出力:<br/>3:4 + 2.35:1 + 16:9/5:2]
