---
name: cover-studio
description: 万能封面工作室 (Cover Studio) - 一站式全平台爆款封面选型与生成 Skill。集成社区 9 大顶级开源封面引擎，智能路由 4 大流派。输入文章后自动确认文字层级（大字标题/副标题/小标签），直接并行输出 3 种不同风格的成图或 Nano Banana (Google Flow 免费平台) 提示词，支持按用户自定义的平台尺寸组合一键批量出图与品牌固化。
---

# 🎨 Cover Studio (万能封面工作室)

`cover-studio` 是一站式解决全平台封面制作与排版困扰的 AI Skill。无需在数十个独立封面工具间纠结，它通过智能路由机制，将社区最成熟的 **9 大开源封面引擎** 融会贯通，提供从**自定义尺寸组合配置 ➔ 文字层级精准确认 ➔ 3 种风格直接出图/出提示词 ➔ 自选尺寸一键批量交付**的极简闭环。
...
## 阶段 0：首次配置出图模式与【自定义尺寸组合】(First-Run Intake)

在用户首次安装或初次使用时，**第一时间与用户确定模式及所需尺寸组合**（仅需配置一次，永久记忆）：

> **“欢迎使用 Cover Studio 万能封面工作室！🐻✨ 为方便后续极速出图，请先告诉我您的常用出图偏好：”**
> 
> 1. 🌟 **【统一品牌多尺寸分发模式】（推荐）**：每次选定 1 个心仪风格后，自动为您**一次性批量输出您自选的多平台自适应尺寸**，全网视觉高度统一。
> 2. 🎯 **【单尺寸独立定制模式】**：每次只针对当前指定的单一画幅单独定制不同风格。

当用户选择 **【统一品牌多尺寸分发模式】** 时，Skill **立刻引导用户自主勾选常驻尺寸组合**：

```markdown
👌 请勾选您日常运营需要常驻批量输出的平台画幅组合（可直接回复编号组合，如 `1+2+3` 或 `全部`）：

- [x] 1. 📕 **小红书 / 抖音图文** (`3:4` 竖版，1080×1440)
- [x] 2. 🟢 **微信公众号首图** (`2.35:1` 横幅，2350×1000，通过 1:1 居中测试)
- [x] 3. 🐦 **X / Twitter / 即刻 / 专栏** (`16:9` 或 `5:2` 宽屏，1200×675)
- [ ] 4. 🎬 **B站 / YouTube 视频封面** (`16:9` 横版，1920×1080)
- [ ] 5. 💻 **GitHub Social Preview / 博客头图** (`2:1` 横版卡片，1280×640)
- [ ] 6. 🔲 **通用方形配图 / 朋友圈** (`1:1` 正方形，1080×1080)

未来每次选定风格后，Cover Studio 将为您一次性批量生成上述全部选中尺寸！
```
...
## 阶段 3：用户挑选 ➔ 自选尺寸全套批量交付与免费出图指引

用户回复选定风格（如“选 B”）后：
1. **按自定义尺寸批量出图（若处于统一品牌模式）**：
   - 自动基于选中的风格 B，一键输出阶段 0 中用户自主勾选的**全部尺寸资产**（例如同时输出小红书 3:4、公众号 2.35:1、X 16:9 的成图或专属 Prompt）！
